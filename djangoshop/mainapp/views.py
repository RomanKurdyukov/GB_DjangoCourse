...
def products(request, pk=None):
    ...
    products = Product.objects.select_related('image').all().order_by('price')[:4]

    if pk is not None:
        if pk == 0:
            products = get_products_order_by_price().select_related('image')
            category = {'name': 'все'}
        else:
            category = get_category(pk)
            products = get_products_in_category_orederd_by_price(pk).select_related('image')
...
def product(request, pk):
    ...
    product = get_object_or_404(Product, pk=pk).select_related("image")
